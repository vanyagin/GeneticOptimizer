...
Проект — дипломная работа (магистратура). Помимо исходного кода, в репозитории
есть готовый комплект для сборки **установщика** и **портативной версии**, чтобы
программу можно было запустить на любом компьютере с Windows без установки Python.
...
- Установщик и портативная версия — интернет на машине пользователя не нужен.

## Быстрый старт

> Для запуска пользователю (например, члену комиссии) **не нужны** ни Python, ни
> Visual Studio — всё уже вложено в установщик и в портативный архив.

### Вариант 1. Установщик
1. Запустить `GeneticOptimizer-Setup.exe`, пройти мастер установки
   (права администратора не требуются).
2. Открыть приложение ярлыком **«Оптимизатор функций»** в меню «Пуск» или на рабочем столе.

### Вариант 2. Портативная версия
1. Распаковать `GeneticOptimizer-portable.zip` в любую папку.
2. Запустить `run.bat`.

В обоих случаях откроется браузер со страницей оптимизатора по адресу
